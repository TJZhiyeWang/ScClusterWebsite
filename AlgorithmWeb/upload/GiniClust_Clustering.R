...
GiniClust_Clustering <- function(ExprM.RawCounts,ExprM.RawCounts.filter,GeneList.final,eps,MinPts,exprimentID){
  ...
  if(data.type == 'RNA-seq'){
    ...
    write.csv(clustering_membership_r, file=paste(exprimentID,"_clusterID.csv", sep=""))
