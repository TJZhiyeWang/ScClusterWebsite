...
class Point extends Chart {
    ...
    initData() {
        var that = this,
            ...
            min, max, numMin, numMax, item, obj, arr = [],arrx=[],minx,maxx,
            numArr = [],
            index = 0,
            r,h;
        ...
        // 计算数据在Y轴刻度
        this.infoy = calculateNum(arr, true);
        min = this.infoy.min;
        max = this.infoy.max;
        //计算数据在X轴刻度
        this.infox = calculateNum(arrx, true);
        minx = this.infox.min;
        maxx = this.infox.max;
        ...
        for (var i = 0; i < this.series.length; i++) {
            ...
            if (!this.animateArr[i]) {
                ...
                item.data.forEach((d, j) => {
                    ...
                    obj.data[j].h = h;
                    obj.data[j].p = h;
                    obj.data[j].x = Math.floor((d.xVal-minx)/ (maxx-minx)* xdis);
                    obj.data[j].y = h;
                    obj.data[j].radius = r;
                    obj.data[j].r = 0;
                    obj.data[j].v = r / 20;
    ...
    drawX(){
       ...
       // xdis = 2 * padding + this.paddingTop - this.H,
        xdis = this.W - this.padding * 2,
    ...
    //画x轴刻度
        ...
        for (var i = 0; i <= xl; i++) {
            ctx.beginPath();
            ctx.strokeStyle = 'hsl(220,100%,50%,1)';
            ctx.moveTo( Math.floor(xs * i),0);
            ctx.lineTo( Math.floor(xs * i),10);
            ctx.stroke();
        ...
        if (i > 0) {
            ...
            ctx.strokeStyle = 'hsla(0,0%,80%,1)';
            ctx.moveTo(Math.floor(xs * i),0);
            ctx.lineTo(Math.floor(xs * i), -ydis);
            ctx.stroke();
        }

        ctx.textAlign = 'right';
        var dim = Math.floor(this.infox.step * i + this.infox.min),
            txt = this.xAxis.formatter ? this.xAxis.formatter.replace('{value}', dim) : dim;
        ctx.fillText(txt, xs * i + 30, 40);
        }
        ctx.restore();
    //画数据
    }
    drawY() {////////////////////////////////////////////////////////////////////////////////////------------------
        ...
        for (var i = 0; i <= yl; i++) {
            ...
            ctx.lineTo(0, -Math.floor(ys * i)); //画刻度线
            ctx.stroke();
            ...
            if (i > 0) {
                ...
                ctx.lineTo(xdis, -Math.floor(ys * i));  //画坐标线
                ctx.stroke();
            }

            ctx.textAlign = 'right';
            var dim = Math.floor(this.infoy.step * i + this.infoy.min),
                txt = this.yAxis.formatter ? this.yAxis.formatter.replace('{value}', dim) : dim;
            ctx.fillText(txt, -20, -ys * i + 10);  //标刻度值
        }
        ctx.restore();
        //画数据
    }
    ...
    drawLine(pos) {
        ...
        xNum = Math.round((xmin + (pos.x * 2 - padding) / xdis * (xmax-xmin)) * 100) / 100;
